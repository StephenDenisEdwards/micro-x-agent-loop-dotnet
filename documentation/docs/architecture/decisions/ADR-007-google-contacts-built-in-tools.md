# ADR-007: Google Contacts as Built-in Tools

## Status

Accepted

## Context

The agent already has Gmail and Google Calendar as built-in tools using `Google.Apis.Gmail.v1` and `Google.Apis.Calendar.v3` with OAuth2 authentication. These follow a consistent pattern: a per-service auth class (token caching, OAuth browser flow), individual tool classes implementing `ITool`, and conditional registration in `ToolRegistry.cs` when Google credentials are present.

The user needs to search, view, create, update, and delete Google Contacts from within the agent. Two approaches were considered:

1. **MCP server** — implement contacts access as an external MCP server (like WhatsApp), communicating over stdio.
2. **Built-in tools** — add contacts tools directly in the agent codebase, following the same pattern as Gmail and Calendar.

## Decision

Implement Google Contacts as built-in tools using the Google People API v1 (`Google.Apis.PeopleService.v1`), following the identical patterns established by Gmail and Calendar.

Reasons:

- **No new dependencies.** The Google APIs NuGet ecosystem is already in `MicroXAgentLoop.csproj`. The People API is just another Google API service package.
- **Consistent architecture.** Gmail and Calendar already proved this pattern works well — same auth class structure, same `ITool` implementation, same `ToolRegistry` registration. Adding a third Google service the same way keeps the codebase predictable.
- **No IPC overhead.** Built-in tools run in-process. An MCP server would add process management, stdio communication, and a separate build/run step for no architectural benefit — this is a first-party Google API, not a third-party service with its own toolchain (see ADR-006).
- **Shared OAuth infrastructure.** The auth class mirrors `CalendarAuth.cs` exactly, just with a different scope and token directory. A future consolidation into a single Google auth module would be straightforward.

The implementation consists of:

- `Tools/Contacts/ContactsAuth.cs` — OAuth2 with `PeopleServiceService.Scope.Contacts`, token cache at `.contacts-tokens/`
- `Tools/Contacts/ContactsFormatter.cs` — shared formatting helpers for summary and detail views
- Six tool classes: `contacts_search`, `contacts_list`, `contacts_get`, `contacts_create`, `contacts_update`, `contacts_delete`

**Prerequisite:** The People API must be enabled in the same Google Cloud project used for Gmail and Calendar.

## Consequences

**Easier:**

- Full CRUD on Google Contacts from natural language (search, list, get, create, update, delete)
- Same OAuth flow users are already familiar with from Gmail/Calendar — first use triggers a browser consent screen
- Etag-based concurrency control on updates prevents accidental overwrites
- No additional processes, config, or dependencies to manage

**Harder:**

- Users must enable the People API in their Google Cloud project (one-time setup)
- A separate OAuth consent flow is required for the contacts scope (separate token cache from Gmail/Calendar)
- Adding more Google APIs in the future will continue to duplicate the auth class pattern — a shared Google auth module may be worth extracting if a fourth service is added

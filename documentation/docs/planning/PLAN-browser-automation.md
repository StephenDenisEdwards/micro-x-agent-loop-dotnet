...
- **Text extraction** — reuse `HtmlUtilities.HtmlToText()` on rendered DOM for consistency with `web_fetch`
- **Timeout per action** — 30s default, same as `web_fetch`

## Implementation

- New file: `src/MicroXAgentLoop/Tools/Web/BrowserTool.cs`
- New dependency: `Microsoft.Playwright` NuGet package (requires `playwright install chromium` post-install)
- Register unconditionally in `ToolRegistry.cs` (lazy browser launch on first use)
...
- `Microsoft.Playwright` — new NuGet dependency

...
These keys should be stored securely (e.g., via environment variables or secret managers) and **never** in config files committed to source control.
...
- **Anthropic:** Official SDKs do not cover Admin API. Use direct HTTP requests via `HttpClient`.
- **OpenAI:** The official SDK does not appear to cover admin endpoints. Use direct HTTP requests.

Both are straightforward REST APIs with JSON payloads, making them good candidates for simple `HttpClient`-based tool implementations.

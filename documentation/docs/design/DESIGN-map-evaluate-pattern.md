...
```csharp
public class EvaluateItemsTool : ITool
{
    /// <summary>
    ...
    public string Name => "evaluate_items";
    ...
    // InputSchema: rubric (object), items (array), output_fields (array), concurrency (int)
}
```
...
const string EvalModel = "claude-sonnet-4-5-20250929";  // Fast, cheap, good at structured output
const int EvalMaxTokens = 1024;                         // Scores are compact
const double EvalTemperature = 0.0;                     // Deterministic scoring
```
...
```csharp
private async Task<List<ScoreResult>> EvaluateAllAsync(
    JsonNode rubric, JsonNode[] items, string[] outputFields)
{
    var semaphore = new SemaphoreSlim(_concurrency); // default 3
    ...
    var tasks = items.Select(async item =>
    {
        ...
        try
        {
            var content = item["content"]?.GetValue<string>()
                ?? await FetchItemContentAsync(item["source"]!.GetValue<string>());
            return await ScoreItemAsync(rubric, content, item, outputFields);
        }
        finally
        {
            semaphore.Release();
...
The evaluate_items tool implements the existing `ITool` interface:
...
No changes needed to `Agent`, `AgentConfig`, `LlmClient`, or the tool protocol.
...
public class EvaluateItemsTool : ITool
{
    ...
    public EvaluateItemsTool(string apiKey, string model = "claude-sonnet-4-5-20250929")
    {
...
Registered in `ToolRegistry.cs` like any other tool, receiving the API key
...
- EvaluateItemsTool class conforming to ITool interface
